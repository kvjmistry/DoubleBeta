...
hits_tot = hits[hits.label == "ACTIVE"]
hits_tot = hits_tot.groupby('event_id')['energy'].sum().reset_index(name='hit_tot_energy')
print(hits_tot)
...
with pd.HDFStore(f"{sys.argv[1]}_slim.h5", mode='w', complevel=5, complib='zlib') as store:
    ...
    store.put('MC/hits_tot',hits_tot, format='table')
